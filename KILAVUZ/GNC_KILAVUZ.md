...
### `filo.alan_tarama_baslat(grup_id, alan, **kwargs)`
...
```python
plan = filo.alan_tarama_baslat(
    grup_id=0,
    alan=(-80, -60, 80, 60, -20),
    derinlik=-20,
    serit_araligi=15,
    gereken_rov_sayisi=4,
    sessiz=False,
)
```
...
```python
filo.alan_tarama_guncelle()
filo.alan_tarama_guncelle(grup_id=0)
filo.alan_tarama_durdur(grup_id=0, lideri_takip_et=True)
```
...
```python
plan = filo.alan_tarama_baslat(0, (-100, -80, 100, 80, -20), gereken_rov_sayisi=4, sessiz=False)
biten = filo.alan_tarama_guncelle()
```
