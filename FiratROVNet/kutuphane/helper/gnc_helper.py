"""
GNC Helper Module
Mathematical calculations, geometric operations, and complex logic for Filo and TemelGNC classes to hlp gnc.py.
"""
...
from FiratROVNet.hull import HullManager as hmr
try:
    import alphashape
...
class Hidrodinamik:
    ...
    # --- Motor Özellikleri ---
    MAX_ITME_KUVVETI = 240.0 # Newton (Toplam motor gücü, örn: T200 motorlar için ~5kgf)
    
    # --- Sürtünme (Drag) Katsayısı ---
    # F_drag = 0.5 * rho * v^2 * Cd * A
    DRAG_KATSAYISI_CD = 0.85  # Kutu gibi şekiller için 0.8 - 1.0 arası
...
class FiloHelper:
    ...
    def get(self, rov_id: int = None, veri_tipi: str = None, taraf: int = None, koordinator=None, sessiz: bool = False):
            """
            ROV bilgilerini güvenli bir şekilde alır.
            
            Args:
                rov_id: ROV ID (0, 1, 2, ...) veya None (tüm ROV'lar için)
                veri_tipi: 'gps', 'hiz', 'batarya', 'rol', 'lidar', 'engels', 'yaw' vb.
                taraf: Lidar yönü (0: Ön, 1: Sağ, 2: Sol)
                koordinator: Koordinat dönüştürücü sınıfı
                sessiz: Hata mesajlarını gizler
            """
            # 1. Toplu GPS İsteği (Tüm ROV'lar)
            if rov_id is None and veri_tipi is None:
                return self.filo._get_all_rovs_positions()

            # 2. Ortam ve Liste Kontrolü
            if not self.filo.ortam_ref or not hasattr(self.filo.ortam_ref, 'rovs'):
                if not sessiz: print("❌ [HATA] Ortam veya ROV listesi bulunamadı!")
                return None

            rov_listesi = self.filo.ortam_ref.rovs
            if len(rov_listesi) == 0:
                if not sessiz: print("❌ [HATA] Henüz hiç ROV yok.")
                return None

            # 3. ID Sınır Kontrolü
            if rov_id is None or not isinstance(rov_id, int) or rov_id < 0 or rov_id >= len(rov_listesi):
                if not sessiz: 
                    print(f"❌ [HATA] Geçersiz ROV ID: {rov_id} (Mevcut: 0-{len(rov_listesi)-1})")
                return None

            # 4. ROV Nesnesini Al ve Canlılık Kontrolü
            rov = rov_listesi[rov_id]
            
            # Eğer ROV listede None ise veya Ursina tarafında silindiyse (is_destroyed)
            if rov is None or (hasattr(rov, 'is_destroyed') and rov.is_destroyed):
                if not sessiz and veri_tipi: 
                    print(f"⚠️ [GET] ROV-{rov_id} ulaşılamaz (silinmiş veya patlamış).")
                return None

            try:
                val = None
                
                # --- ÖZEL VERİ TİPİ İŞLEMLERİ ---
                
                if veri_tipi == "lidar":
                    # ROV sınıfında 'get' metodu lidar için parametre almayabilir,
                    # bu yüzden doğrudan property üzerinden okuyoruz.
                    if hasattr(rov, 'son_lidar_mesafeleri'):
                        if taraf is not None:
                            val = rov.son_lidar_mesafeleri.get(taraf, -1)
                        else:
                            # Taraf belirtilmezse en yakın engeli döndür
                    ...
                    # Engel hesaplama fonksiyonunu çağır
                    val = self.compute_obstacle_positions(rov_id)

                else:
                    # Diğer standart veriler (batarya, rol, hiz, yaw, sonar...)
                    # ROV.get() metoduna yönlendir
                    val = rov.get(veri_tipi)

                if val is None and not sessiz:
                    # Debug amaçlı log (Sık çağrılan verilerde kapatılabilir)
                    # print(f"⚠️ [GET] ROV-{rov_id} '{veri_tipi}' verisi None döndü.")
                    pass

                return val

            except Exception as e:
                # Obje o an silindiyse Panda3D hatası verebilir, bunu sessizce yutuyoruz
                if "!is_empty()" not in str(e):
                    print(f"❌ [HATA] Helper.get hatası ({veri_tipi}): {e}")
                import traceback
                # traceback.print_exc() # Detaylı hata gerekirse açılabilir
                return None
    ...
    def engel_bul(self, rov_id: int, menzil: float = None, debug: bool = False) -> list:
                """
                ROV için 3D çevresel tarama yapar.
                DÜZELTME: Yeni mimaride ortam.rovs listesi doğrudan Entity'leri tutar.
                """
                import math
                ...
                # --- DÜZELTME BAŞLANGICI ---
                # ROV Nesnesine Eriş (Doğrudan listeden al)
                if not self.filo.ortam_ref or not hasattr(self.filo.ortam_ref, 'rovs'):
                    return []
                    
                # ID kontrolü
                if not (0 <= rov_id < len(self.filo.ortam_ref.rovs)):
                    return []

                rov = self.filo.ortam_ref.rovs[rov_id]
                ...
                for ad, yon in tarama_yonleri.items():
                    ...
                    if hit.hit:
                        ...
                        # Simülasyon: (x, y_ileri, z_derinlik)
                        # Ursina: (x, y_yukari, z_ileri)
                        # Dönüşüm: Sim(x, z, -y)
                        sim_koord = (pt.x, pt.z, pt.y) # Z ekseni Ursina'da ileri, Y yukarı
                        ...
                        res = {
                            ...
                            'radius': radius,
                            'entity': hit.entity,
                            # Ursina koordinatını da sakla (Vektör hesabı için gerekli olabilir)
                            'ursina_pos': pt 
                        }
                        sonuclar.append(res)
                        ...
                        if debug:
                            ...
                            if len(self._debug_ents) > 50: destroy(self._debug_ents.pop(0))
    ...
    def apf(self, rov_id: int, hedef: bool = True, engel: bool = True, rov: bool = True):
                """
                3D APF Hesaplama. Engel kaçınma düzeltmeleri yapıldı.
                """
                ...
                if engel:
                    ...
                    for e in tespit_edilenler:
                        # 'koordinat' anahtarı simülasyon koordinatıdır (x, z, derinlik)
                        target = e.get('koordinat') 
                        sensor_mesafesi = float(e.get('mesafe', 0.0))
                        
                        if target:
                            # Vektörü hesapla (reverse=True -> İtme Kuvveti)
                            res = self.vektor(
                                rov_id_ilk=rov_id, 
    ...
    def _git_impl(self, rov_id: int, x: float, y: float, z: float = None, ai: bool = True, sessiz: bool = True) -> None:
            """
            git() fonksiyonunun yeni mimariye (rov.gnc) uyarlanmış implementasyonu.
            """
            # 1. ORTAM VE LİSTE KONTROLLERİ
            if not self.filo.ortam_ref or not hasattr(self.filo.ortam_ref, 'rovs'):
                if not sessiz: print("❌ [FİLO] Ortam referansı bulunamadı.")
                return

            rov_listesi = self.filo.ortam_ref.rovs

            # 2. ID VE CANLILIK KONTROLÜ
            if not (0 <= rov_id < len(rov_listesi)):
                if not sessiz: print(f"❌ [FİLO] Geçersiz ROV ID: {rov_id}")
                return

            rov = rov_listesi[rov_id]
            
            # ROV yoksa veya patlamışsa işlem yapma
            if rov is None or (hasattr(rov, 'is_destroyed') and rov.is_destroyed):
                return

            # ROV'un GNC sistemi var mı?
            if not hasattr(rov, 'gnc') or rov.gnc is None:
                if not sessiz: print(f"❌ [FİLO] ROV-{rov_id} için GNC sistemi bulunamadı.")
                return

            # 3. MANUEL DERİNLİK HESABI (Z verilmemişse)
            if z is None:
                # Mevcut konumdan Z'yi çek (Koordinator kullanarak tutarlılık sağla)
                # Ursina (x, y, z) -> Sim (x, y, z_depth)
                # Genelde Ursina Y = Sim Z (derinlik)
    ...
    def git(self, rov_id: int, x, y: float = None, z: float = None, ai: bool = True, sessiz: bool = False) -> None:
            """
            ROV'a hedef koordinatı atayan genel fonksiyon.
            ...
            if isinstance(x, (list, tuple)):
                ...
                if isinstance(x[0], (list, tuple)):
                    # Rota listesini Filo hafızasına kaydet
            ...
            # 2. UYGULAMA (Implementasyona yönlendir)
    ...
    def move(self, rov_id: int, yon: str, guc: float = 1.0, sessiz: bool = True) -> None:
        ...
        if not isinstance(rov_id, int) or rov_id < 0:
            if not sessiz:
                ...
                print(f"   Mevcut ROV sayısı: {len(self.filo.ortam_ref.rovs)} (0-{len(self.filo.ortam_ref.rovs)-1} arası)")
            return

        if rov_id >= len(self.filo.ortam_ref.rovs):
            if not sessiz:
        ...
        try:
            ...
            gnc = self.filo.ortam_ref.rovs[rov_id]
            rov = gnc.rov
            ...
            if not sessiz:
                ...
                print(f"   💡 Debug: GNC sistemi tipi: {type(self.filo.ortam_ref.rovs[rov_id])}")
                import traceback
                traceback.print_exc()
        except Exception as e:
            if not sessiz:
    ...
    def formasyon(self, formasyon_id="LINE", aralik=None, is_3d=False, lider_koordinat=None, dinamik=True):
        ...
        if len(pozisyonlar) != len(self.filo.ortam_ref.rovs):
            print(f"⚠️ [FORMASYON] Uyarı: Pozisyon sayısı ({len(pozisyonlar)}) ROV sayısı ({len(self.filo.ortam_ref.rovs)}) ile eşleşmiyor!")
        ...
        # Lideri bul
        lider_id = 0
        for i, sistem in enumerate(self.filo.ortam_ref.rovs):
             if hasattr(sistem, 'rov') and sistem.rov.role == 1:
                 lider_id = i
                 break

        for i, pozisyon in enumerate(pozisyonlar):
            if i >= len(self.filo.ortam_ref.rovs):
                break
    ...
    def _formasyon_sec_impl(self, margin=None, is_3d=False, offset=None, sessiz=True, dinamik=True,tekrar=1,g_id=0):
            ...
            try:
                ...
                # --- YENİ: YASAKLI NOKTALARI BİRLEŞTİR (Ada Çevresi + Engel Bulutu) ---
                # 1. Sabit engeller (Adalar)
                yasakli_noktalar = self.filo.ada_cevre() 

                # 2. Dinamik engeller (Lidar/Sonar ile tespit edilen engel bulutu)
                if self.filo.ortam_ref and hasattr(self.filo.ortam_ref, 'engel_bulutu'):
                    # engel_bulutu (x, z) çiftlerinden oluşur, bunları yasaklı listesine ekliyoruz
                    dinamik_engeller = [[float(p[0]), float(p[1])] for p in self.filo.ortam_ref.engel_bulutu]
                    yasakli_noktalar.extend(dinamik_engeller)
                
                # Opsiyonel: O anki tüm ROV'ların engel_bul() sonuçlarını da anlık eklemek isterseniz:
                # for r_id in range(len(self.filo.ortam_ref.rovs)):
    ...
    def _apply_formation_results(self, f_id, aralik, yaw, merkez, pozisyonlar, lider_id, is_3d, dinamik, sessiz, lider_hareket_halinde, g_id=0):
            """
            Hesaplaması bitmiş formasyonu uygular.
            
            Args:
                ...
                pozisyonlar: Liste [(x,y,z)...] VEYA Sözlük {rov_id: (x,y,z)...} olabilir.
                g_id (int): Hangi gruba uygulanacağı (Varsayılan: 0).
            """
            
            # 1. HEDEF GRUBU BELİRLE
            # self.filo.g_rovs[g_id] -> [RovEntity1, RovEntity2...] listesini verir
            ...
            # 2. GİRDİ VERİSİNİ NORMALİZE ET (Sözlük formatına çevir: {rov_id: (x,y,z)})
            # Böylece hem liste hem sözlük gelse de kod aşağıda aynı çalışır.
            target_positions = {}
            
            if isinstance(pozisyonlar, dict):
                # Eğer veri zaten sözlükse (yeni yapı), olduğu gibi kullan
                target_positions = pozisyonlar
            elif isinstance(pozisyonlar, list):
                # Eğer veri listeyse (eski yapı), gruptaki ROV sırasına göre eşleştir
                for idx, rov in enumerate(group_rov_list):
                    if idx < len(pozisyonlar):
                        target_positions[rov.id] = pozisyonlar[idx]

            # 3. FORMASYON BİLGİSİNİ KAYDET
            # Dinamik takip için kaydederken g_id bilgisini de ekleyebiliriz (opsiyonel)
            if dinamik:
            ...
            # 4. HEDEFLERİ UYGULA
            # Artık target_positions kesinlikle {rov_id: (x,y,z)} formatındadır.
            for r_id, pos in target_positions.items():
                ...
                # Koordinat Çözümleme (Unpack Hatası Burada Çözüldü)
                try:
                ...
                # Z Ekseni Ayarı (Yüzey kontrolü)
                final_z = -10.0 if sim_z >= 0 else sim_z
                
                # Takipçiler için hedef kaydı (Navigasyon hafızası için)
                if r_id != lider_id:
                    # self.filo._formasyon_hedefleri genelde dict tabanlıdır, ID kullanırız
                ...
                # Komutu Gönder (Entity ID kullanılarak)
                # self.filo.git fonksiyonunun ID kabul ettiğini varsayıyoruz
                self.filo.git(r_id, sim_x, sim_y, final_z, ai=True, sessiz=sessiz)


    def ada_cevre(self, offset: float = 15.0, sessiz: bool = False) -> list:
            """
            Simülasyondaki adaları tespit edip 2D (X, Z) düzleminde noktalar döndürür.
            Hull algoritması için [x, z] formatında çıktı verir.
            """
            if not self.filo.ortam_ref:
                if not sessiz:
            ...
            for island_data in self.filo.ortam_ref.island_positions:
                ...
                # 12 Nokta ile daireyi simüle et
                for i in range(12):
                    aci_rad = math.radians(i * 30) # 360 / 12 = 30 derece
                    
                    # Trigonometri ile çember üzerindeki nokta (X, Z Düzlemi)
                    nx = island_x + cevre_mesafesi * math.sin(aci_rad)
                    nz = island_z + cevre_mesafesi * math.cos(aci_rad)
                    
                    # DÜZELTME: [x, z] formatında 2D liste döndür. 
                    # (Eskiden (x, z, 0) dönüyordu, bu da eksen karmaşası yaratıyordu)
    ...
    def yeni_hull(self, yasakli_noktalar=None, offset=50.0, buffer_radius=10.0, g_id=0, **kwargs):
            """
            Lider ROV etrafında dairesel bir güvenli alan (Hull) oluşturur.
            """
            import math
            
            # 1️⃣ Base Points (Temel Noktalar - Daire) Hazırla
            base_points = []
            ...
            if lider_gps is None:
                # Lider yoksa merkez (0,0) kabul et ve uyarı ver
                print(f"⚠️ [UYARI] Grup-{g_id} için Lider ROV bulunamadı! Merkez (0,0) kabul ediliyor.")
                lx, ly = 0.0, 0.0 
            else:
                lx, ly = lider_gps[0], lider_gps[1]

            # Daire oluştur (Lider merkezli)
            # Offset değerini yarıçap olarak kullanıyoruz (Min 15m)
            radius = max(15.0, float(offset))
            
            # 16 genli bir daire oluşturuyoruz
            for i in range(16):
            ...
            # 2️⃣ HullManager'a Erişim
            # Varsa filodaki mevcut manager'ı kullan, yoksa yeni oluştur
            mgr = None
            if hasattr(self.filo, 'hull_manager') and self.filo.hull_manager:
                ...
                try:
                    ...
                    print("❌ [KRİTİK] HullManager modülü bulunamadı!")
                    return {'points': None, 'center': None, 'hull': None}

            # 3️⃣ Hesaplamayı Yap
            # base_points listesi (daire noktaları) gönderiliyor.
            # offset=0.0 gönderiyoruz çünkü zaten yarıçap (radius) ile genişliği belirledik.
            sonuc = mgr.yeni_hull(
            ...
            if sonuc.get('points') is None:
                print(f"❌ [DEBUG] HullManager Grup-{g_id} için sonuç döndüremedi!")

            return sonuc

    def find_leader_info(self, sessiz: bool = False, g_id: int = 0) -> tuple:
        """
        Belirtilen g_id grubundaki Lider ROV'un ID ve GPS bilgisini döndürür.
        Returns: (lider_id, (x, y, z))
        """
        lider_rov_id = None
        lider_gps = None
        
        # Gruptaki ROV listesini al (Dictionary üzerinden)
        ...
        # Liste üzerinde dön (enumerate indexini ID sanma hatası düzeltildi)
        for rov in rov_listesi:
            if rov is None: continue
            
            # --- Rol Kontrolü (Fallback mekanizması) ---
            rol = -1
            # 1. Entity üzerindeki 'rol' özelliği
            if hasattr(rov, 'rol'):
                rol = rov.rol
            # 2. Entity üzerindeki 'role' özelliği (İngilizce versiyon)
            elif hasattr(rov, 'role'):
                rol = rov.role
            # 3. .get() metodu (Eski sözlük yapısı)
            ...
            # Lider bulundu mu? (Rol == 1)
            if rol == 1:
                lider_rov_id = rov.id
                
                # GPS verisini al (Entity ID kullanarak)
                gps = None
                
                # GPS alma yöntemleri
                if hasattr(self.filo, 'get'):
                    gps = self.filo.get(rov.id, "gps")
                elif hasattr(rov, 'gps'): # Entity üzerinde direkt gps varsa
                    gps = rov.gps
                
                if gps:
                    # Tuple dönüşümü ve float güvenliği
                    try:
                ...
                break # Lider bulundu, döngüden çık
        
        if lider_gps is None and not sessiz:
            # Hata ayıklama için ds varsa yazdır
...
class TemelGNCHelper:
    ...
    def fizik_uygula(self, hedef_yon_ursina: Vec3, guc_orani: float, dt: float, uygula: bool = True):
        ...
        if hiz_buyuklugu > 0.001:
            ...
            drag_linear = hiz_buyuklugu * 10.0 # Sabit bir sönümleme katsayısı
    ...
    def vektor_to_motor_sim(self, v_sim_dir: Vec3, guc_orani: float):
        ...
        #print(hesaplanan_hiz,self.rov.y)
    ...
    def _formasyon_dinamik_guncelle(self, rov_id: int):
            ...
            if self.rov.role == 1:
                ...
                self.filo_ref.yeni_pozisyonlar = f_obj.pozisyonlar(
                    aktif['id'], 
                    aktif['aralik'], 
                    is_3d=aktif['is_3d'], 
            ...
            #print(self.filo_ref.yeni_pozisyonlar)


            # 2. Bu ROV'un payına düşen hedefi güncelle
            if self.filo_ref.yeni_pozisyonlar is not None and rov_id < len(self.filo_ref.yeni_pozisyonlar):
                hedef = self.filo_ref.yeni_pozisyonlar[rov_id]
